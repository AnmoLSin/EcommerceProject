...
firebase.auth().setPersistence(firebase.auth.Auth.Persistence.LOCAL)
  .then(function() {
    ...
    return firebase.auth().signInWithEmailAndPassword(input_name.value, input_pwd.value).then(function(){window.location.assign('ndex.html')},function(error) {
    alert(error.message);
})
  })
...
if(Email.value==''||Pw.value==""||Rep.value=="")
{alert('Field cannot be empty');
        return false;}
...
checkForm();
   ...
   var users=getStoredProducts();
    // entered data from the login-form
    var userName = document.getElementById('userName');
    var userPw = document.getElementById('userPw');
for(var j=0;j<users.length;j++)
{
    if(users[j].EMAIL==userName.value && users[j].PW==userPw.value)
        {
         alert('Welcome');
          window.open("add to cart.html");
          break;
        }
    else alert('No user found please signup');
}
}
function putArray(users_info)
    {
       users[i]=Object.assign({},users_info);
        i++;
        store(users);
}
